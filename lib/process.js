...
function processPost(data) {
    ...
    if (process.argv.indexOf('generate') > -1 || process.argv.indexOf('g') > -1) {
        ...
        console.log("asset_dir: " + data.asset_dir, " slug: ", data.slug, " data.path: ", data.path, " source: ", data.source, " full_source: ", data.full_source);
        if(typeof data.asset_dir !== "undefined" && typeof this.config !== "undefined") {
            ...
            let postUrlPrefix = urljoin(cdnConfig.cdn_url_prefix, postAssetDirName);
